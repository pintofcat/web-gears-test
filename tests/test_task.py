...
def test_update_task(auth_client, create_user):
    task = Task.objects.create(title='Initial Task', description='Task description', assigned_to=create_user)
...
def test_delete_task(auth_client, create_user):
    task = Task.objects.create(title='Task to Delete', description='Delete this task', assigned_to=create_user)
    url = reverse('tasks-detail', args=[task.id])
